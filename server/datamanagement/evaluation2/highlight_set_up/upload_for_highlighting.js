...
counter = 0
addedCounter= 0
dfd.readExcel('generated_clusters_cleaned.xlsx').then((df) => {
    console.log("loaded")
...
loadDataOfRow = (df, currentRow) => {
    ...
    if (currentRow < df.shape[0] ) {
        if (df.at(currentRow, "my_clust") != "NONE") {


            let history = getHistory(df.at(currentRow, "Antworthistorie_anonym"))

            let datesplitted = df.at(currentRow, "Datum").toString().split(".")

            let date = new Date(datesplitted[2], datesplitted[1] - 1, datesplitted[0])
            ...
            ticket = new model.tickets({
                ...
                category_2: df.at(currentRow, "Kategorie 2"),
                question: df.at(currentRow, "Frage_anonym_cleaned").toString().replace(/  +/g, ' ').replaceAll("<", "").replaceAll(">", "").replaceAll("\r\r\n", "\n").replaceAll("\n\n", "\n").replaceAll("\r\n", "\n"),
                answerhistory: history,
                recommendedTickets: [],
                user_evaluation_shown:0,
